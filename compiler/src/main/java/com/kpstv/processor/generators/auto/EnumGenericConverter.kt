...
object EnumGenericConverter {
    fun generate(typeSpecBuilder: TypeSpec.Builder, typeName: TypeName, packageName: String) {
        val className = typeName.toString().replace("$packageName.", "").replace(".","$")
        val encodeMethodBuilder = MethodSpec.methodBuilder(Consts.toConvertMethod + className)
        ...
        val decodeMethodBuilder = MethodSpec.methodBuilder(Consts.fromConvertMethod + className)
            .addModifiers(Modifier.PUBLIC, Modifier.STATIC)
            .addAnnotation(Consts.CLASSNAME_TYPECONVERTER)
